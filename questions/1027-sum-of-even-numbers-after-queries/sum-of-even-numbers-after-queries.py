...
# <SUBID:319942335,UPDATE:20230205>
# English:
...
class Solution:
    def sumEvenAfterQueries(self, nums: List[int], queries: List[List[int]]) -> List[int]:
        ret = []
        total_even = sum(x for x in nums if x % 2 == 0)   # 还是前所有偶数的和
        for i, (val, index) in zip(nums, queries):
            if nums[index] % 2 == 0:  # 这个数原先是偶数, 先去掉它
                total_even -= nums[index]
            nums[index] += val
            if nums[index] % 2 == 0:  # 增加后还是偶数
                total_even += nums[index]
